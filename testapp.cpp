...
{
    TcpServerController *ctrlr = NULL;
...
{
    printf("[%s,%d]\n", network_convert_ip_n_to_p(htonl(tcp_client->ip_addr), 0), (tcp_client->port_no));
}

static void print_server(const TcpServerController *tcp_server)
{
    printf("[%s,%d]\n", network_convert_ip_n_to_p((tcp_server->ip_addr), 0), htons(tcp_server->port_no));
}
