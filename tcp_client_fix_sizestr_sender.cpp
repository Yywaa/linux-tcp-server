...
void send_msg(int sock, const unsigned char *payload, uint16_t payload_len);

void send_msg(int sock, const unsigned char *payload, uint16_t payload_len)
{
    // int sockfd = 0;
    //  calculate the length of total data
    uint16_t total_len = sizeof(uint16_t) + payload_len;
    ...
    memcpy(send_buf, &len_net, sizeof(uint16_t));

    // wirte payload into send buffer
    memcpy(send_buf + sizeof(uint16_t), payload, payload_len);
    send(sock, send_buf, total_len, 0);
}

void setup_tcp_communicatiom()
{
